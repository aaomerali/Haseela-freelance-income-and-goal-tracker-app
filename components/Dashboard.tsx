    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className={state.language === 'ar' ? 'text-right' : 'text-left'}>
          ...
          <p className="text-gray-400 text-sm">{t.monthlySummary} {t.months[currentMonth - 1]}</p>
        </div>
      ...
      <div className="bg-indigo-600 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col items-center text-center">
          <p className="text-indigo-100 text-sm font-bold mb-1 uppercase">{t.achievedBalance}</p>
          ...
          <div className="flex justify-between w-full text-xs font-bold text-indigo-100">
             <span>{t.goal}: {state.currency}{currentGoal?.targetAmount.toLocaleString() || 0}</span>
             <span className="bg-white bg-opacity-20 px-3 py-1 rounded-full">{progressPercentage}%</span>
          </div>
        </div>
        <button 
          onClick={onSetGoalClick} 
          className={`absolute top-6 ${state.language === 'ar' ? 'left-6' : 'right-6'} w-8 h-8 bg-white bg-opacity-20 rounded-full flex items-center justify-center btn-active`}
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <span className="text-gray-400 text-xs font-bold">{t.completed}</span>
        ...
        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <span className="text-gray-400 text-xs font-bold">{t.ongoing}</span>
          <span className="block text-xl font-bold text-gray-800">
            {state.clients.reduce((acc, c) => acc + c.tasks.filter(t => !t.isCompleted).length, 0)}
          </span>
        </div>
      ...
      <section className="space-y-4">
        <h3 className="text-lg font-bold text-gray-800 px-2">{t.topClients}</h3>
        <div className="space-y-3">
          {totalClientsEarnings.length === 0 ? (
            <div className="bg-white border border-gray-100 rounded-3xl p-8 text-center shadow-sm text-gray-400">...</div>
          ) : (
            totalClientsEarnings.slice(0, 4).map(client => (
              <div key={client.id} onClick={() => onClientClick(client.id)} className="bg-white p-4 rounded-3xl border border-gray-50 flex items-center justify-between shadow-sm btn-active cursor-pointer">
                <div className={`flex items-center ${state.language === 'ar' ? 'space-x-4 space-x-reverse' : 'space-x-4'}`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 font-bold uppercase">{client.tasks.length} {t.tasks}</p>
                  </div>
                </div>
                <div className={state.language === 'ar' ? 'text-left' : 'text-right'}>
